...
#testing datasets AND test methods
...
#features <- c("w1", "w2", "w3", "c3", "c4", "c5", "c6", "c7")
total_feature_results <- data.frame(is_100=rep(0, 8), is_95=rep(0, 8), is_90=rep(0, 8), is_85=rep(0, 8), is_80=rep(0, 8), is_79=rep(0, 8), row.names = features)
    ...
    for (x in 1:length(features)) {
      ...
      for (y in 1:length(file_locs)) {
        ...
        for(result_i in 1:length(temp_results)) {
          ...
          total_feature_results[grep(features[x], row.names(total_feature_results)),grep(paste0("is_", accuracy_group), names(total_feature_results))] <-
            total_feature_results[grep(features[x], row.names(total_feature_results)),grep(paste0("is_", accuracy_group), names(total_feature_results))] + 1
          # results[grep(current_test, row.names(results)),grep(paste0("is_", accuracy_group), names(results))] <-
          #   results[grep(current_test, row.names(results)),grep(paste0("is_", accuracy_group), names(results))] + 1
          # #add to total_model_results table
          # total_model_results[grep(current_test, row.names(results)),grep(paste0("is_", accuracy_group), names(results))] <-
          #   total_model_results[grep(current_test, row.names(results)),grep(paste0("is_", accuracy_group), names(results))] + 1
          next
        }
...
total_mfw_results <- data.frame(is_100=rep(0, 10), is_95=rep(0, 10), is_90=rep(0, 10), is_85=rep(0, 10), is_80=rep(0, 10), is_79=rep(0, 10), row.names = mffeatures)
  ...
  for (i in 1:length(batch_tests)) {
    ...
    for (x in 1:length(result_files)) {
      ...
      for(result_i in 1:length(temp_results)) {
        ...
        total_mfw_results[result_i,grep(paste0("is_", accuracy_group), names(total_mfw_results))] <-
          total_mfw_results[result_i,grep(paste0("is_", accuracy_group), names(total_mfw_results))] + 1
      }
    }
  }
}
...
#make df
clean_names <- c("Delta", "KNN", "NB", "NSC", "SVM")
model_df <- data.frame(
  attributions = c(total_model_results[1,1], total_model_results[1,2], total_model_results[1,3], total_model_results[1,4], total_model_results[1,5], total_model_results[1,6]), 
  Classification = clean_names[1],
  accuracy = c(100,95,90,85,80,79))


for (i in 2:5) {
  model_df <- rbind(model_df,
                    data.frame(attributions = c(total_model_results[i,1], total_model_results[i,2], total_model_results[i,3], total_model_results[i,4], total_model_results[i,5],
                                                total_model_results[i,6]), 
                               Classification = clean_names[i],
                               accuracy = c(100,95,90,85,80,79)))
}
...
clean_names <- toupper(row.names(total_feature_results))
model_df <- data.frame(
  attributions = c(total_feature_results[1,1], total_feature_results[1,2], total_feature_results[1,3], total_feature_results[1,4], 
                   total_feature_results[1,5], total_feature_results[1,6]), 
  Feature = clean_names[1],
  accuracy = c(100,95,90,85,80,79))


for (i in 2:8) {
  model_df <- rbind(model_df,
                    data.frame(attributions = c(total_feature_results[i,1], total_feature_results[i,2], total_feature_results[i,3], total_feature_results[i,4], total_feature_results[i,5],
                                                total_feature_results[i,6]), 
...
model_df <- data.frame(
  attributions = c(total_mfw_results[1,1], total_mfw_results[1,2], total_mfw_results[1,3], total_mfw_results[1,4], 
                   total_mfw_results[1,5], total_mfw_results[1,6]), 
  
  Frequency = mffeatures[1],
  accuracy = c(100,95,90,85,80,79))

for (i in 2:10) {
  model_df <- rbind(model_df,
                    data.frame(attributions = c(total_mfw_results[i,1], total_mfw_results[i,2], total_mfw_results[i,3], total_mfw_results[i,4], total_mfw_results[i,5],
                                                total_mfw_results[i,6]), 
                               Frequency = mffeatures[i],
                               accuracy = c(100,95,90,85,80,79)))
}
...
#FOR THREE DATASETS
clean_names <- c("Small samples", "Large samples", "Texts")

model_df <- data.frame(
  attributions = c(total_dataset_results[1,1], total_dataset_results[1,2], total_dataset_results[1,3], total_dataset_results[1,4], 
                   total_dataset_results[1,5], total_dataset_results[1,6]), 
  Dataset = clean_names[1],
  accuracy = c(100,95,90,85,80,79))

for (i in 2:3) {
  model_df <- rbind(model_df,
                    data.frame(attributions = c(total_dataset_results[i,1], total_dataset_results[i,2], total_dataset_results[i,3], total_dataset_results[i,4], total_dataset_results[i,5],
                                                total_dataset_results[i,6]), 
                               Dataset = clean_names[i],
                               accuracy = c(100,95,90,85,80,79)))
}
