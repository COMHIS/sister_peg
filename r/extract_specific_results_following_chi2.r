...
dataset_test <- c("test_10")
dataset_test <- c("test_11")
dataset_test <- c("test_10", "test_11")
...
#CHOOSE WHICH SIS-PEG TEST
sp_test <- "bumbo_13" #ALL BUMBO
sp_test <- "bumbo_no_13" #SUS BUMBO
sp_test <- "no_bumbo_all" #NO BUMBO
sp_test <- "no_bumbo_with" #NONE SUS

for (i_tests in 1:length(dataset_test)) {
  if (dataset_test[i_tests] == "test_10") {
    batch_tests <- c("small_samp_no_cull", "texts_no_cull")
    features <- c("1w", "2w", "3w", "3c", "4c", "5c", "6c", "7c")
    #features <- c("1w", "2w", "3c", "7c")
    models <- c("svm")
    mfws <- c(100, 200, 300, 400, 500, 600, 700, 800, 900, 1000)
  } else if (dataset_test[i_tests] == "test_11") {
    batch_tests <- c("small_samp_no_cull", "texts_no_cull")
    features <- c("1w", "2w", "3w", "3c", "4c", "5c", "6c", "7c")
    #features <- c("1w", "2w", "3c")
    models <- c("svm")
  ...
  for (i in 1:length(batch_tests)) {
    ...
    for (y in 1:length(result_files)) {
      ...
      while(testing == TRUE) {
        ...
        #sp_test <- "no_bumbo_all" #NO BUMBO
        #sp_test <- "no_bumbo_with" #NONE SUS
        if (any(!grepl(sp_test, temp_results$V1) == TRUE)) {
          temp_results <- temp_results[-c(which(!grepl(sp_test, temp_results$V1))),]
        }
